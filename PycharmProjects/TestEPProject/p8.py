...
try:
    for truckD in fleetD:
        ...
        stm = "INSERT INTO TRUCKS VALUES('{0}','{1}')".format(truckD["id"] , truckD["dest"] )
        cur.execute(stm)
        for c in truckD["cargo"]:
            stm = "INSERT INTO CARGO VALUES('{}','{}')".format( truckD["id"] ,str(c))
            cur.execute(stm)

    cur.execute("select id,dest,cargo_box from trucks a, cargo b where a.id=b.truck_id order by a.id")
